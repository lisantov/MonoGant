...
const onBarClick = (e: MouseEvent) => {
    ...
    const sprintName =
        sprints.sprints.value.find((s) => s.id === currentBar.value.sprint_id)?.name ?? 'Спринт';
    openTaskDetail({ taskId: currentBar.value.id, sprintName });
};
